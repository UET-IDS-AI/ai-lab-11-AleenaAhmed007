...
def compute_kmeans_objective(X, centroids, labels):
    ...
    """
    # For each point, get its assigned centroid, compute squared distance
...
def choose_elbow_k(k_values, objectives):
    ...
    x1, y1 = k_arr[0], obj_arr[0]
    x2, y2 = k_arr[-1], obj_arr[-1]
    ...
    # Distance = |( (p - a) x d )| / |d|  (2D cross product)
    distances = []
    for i in range(len(k_arr)):
        px = k_arr[i] - x1
        py = obj_arr[i] - y1
        # 2D cross product magnitude = |px*dy - py*dx|
...
def identify_outliers_by_distance(X, centroids, labels, top_n=5):
    ...
    """
    # Compute squared distance of each point from its assigned centroid
